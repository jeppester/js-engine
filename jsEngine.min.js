...
Rectangle.prototype.contains=function(object){var polygon=this.getPolygon();return polygon.contains(object);};Rectangle.prototype.drawCanvas=function(c){var i,len;c.save();c.strokeStyle="#f00";c.moveTo(this.x,this.y);c.lineTo(this.x+this.width,this.y);c.lineTo(this.x+this.width,this.y+this.height);c.lineTo(this.x,this.y+this.height);c.lineTo(this.x,this.y);c.stroke();c.restore();};NewObject('Polygon',[View]);Polygon.prototype.polygon=function(points){this.view();this.setFromPoints(points);};Polygon.prototype.setFromPoints=function(points){this.points=points;return this;};Polygon.prototype.setFromCoordinates=function(x1,y1,x2,y2,x3,y3){var numPoints,i,x,y;numPoints=Math.floor(arguments.length/2);this.points=[];for(i=0;i<numPoints;i++){x=arguments[i*2];y=arguments[i*2+1];if(typeof x!=='number'||typeof y!=='number'){throw new Error('All arguments should be of type: Number');}
