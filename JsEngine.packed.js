...
this.arena=document.getElementById('arena');this.autoResize=true;this.autoResizeLimitToResolution=true;this.compositedDepths=[];this.gameClassPath="js/Game.js";this.backgroundColor="#FFF";this.timeFactor=1;this.options=_opt?_opt:{};copyOpt=['backgroundColor','arena','loopSpeed','loopsPerColCheck','manualRedrawDepths','compositedDepths','canvasResX','canvasResY','autoResize','autoResizeLimitToResolution','enginePath','themesPath','gameClassPath'];for(i=0;i<copyOpt.length;i++){opt=copyOpt[i];if(this.options[opt]!==undefined){this[opt]=this.options[opt];delete this.options[opt];}}
...
this.startMainLoop();console.log('JsEngine ready');};JsEngine.prototype.autoResizeCanvas=function(){var h,w,windowWH,gameWH;windowWH=window.innerWidth/window.innerHeight;gameWH=this.canvasResX/this.canvasResY;if(windowWH>gameWH){h=window.innerHeight;w=this.canvasResX/this.canvasResY*h;}else{w=window.innerWidth;h=this.canvasResY/this.canvasResX*w;}
...
this.gameTime+=this.timeIncrease;this.executingLoops=false;this.lastLoopTime=this.now-this.last;this.last=this.now;engine.redraw(0);if(this.fpsMsCounter<1000){this.fpsCounter++;this.fpsMsCounter+=this.lastLoopTime;}
...
this.themes[name]=theme;theme.ressourcesCount=0;theme.ressourcesLoaded=0;this.loadRessources(theme,theme.images,'images');this.loadRessources(theme,theme.sfx,'sfx');this.loadRessources(theme,theme.music,'music');}
...
if(typeString===undefined){throw new Error('Missing argument: typeString');}
var onload,res,path,i,format;onload=function(){var total,loaded,theme,i;if(this.hasAttribute('data-loaded')){return;}
...
ctx.putImageData(bitmap,0,0);this.mask=canvas;};Collidable.prototype.boundingBoxCollidesWith=function(objects,getCollidingObjects){if(objects===undefined){throw new Error('Missing argument: objects');}
if(!Array.prototype.isPrototypeOf(objects)){objects=[objects];}
getCollidingObjects=getCollidingObjects!==undefined?getCollidingObjects:false;var obj,bb1,bb2,i,collideObjects;bb1={x1:this.x+this.boundingBox.xOff,x2:this.x+this.boundingBox.xOff+this.boundingBox.width,y1:this.y+this.boundingBox.yOff,y2:this.y+this.boundingBox.yOff+this.boundingBox.height};collidingObjects=[];for(i=0;i<objects.length;i++){obj=objects[i];bb2={x1:obj.x+obj.boundingBox.xOff,x2:obj.x+obj.boundingBox.xOff+obj.boundingBox.width,y1:obj.y+obj.boundingBox.yOff,y2:obj.y+obj.boundingBox.yOff+obj.boundingBox.height};if(bb1.x1<bb2.x2&&bb1.x2>bb2.x1&&bb1.y1<bb2.y2&&bb1.y2>bb2.y1){if(getCollidingObjects){collidingObjects.push(obj);}
else{return true;}}}
if(collidingObjects.length){return collidingObjects;}
else{return false;}};Collidable.prototype.bitmapCollidesWith=function(objects,resolution,getCollisionPosition,checkBoundingBox){if(objects===undefined){throw new Error('Missing argument: objects');}
if(!Array.prototype.isPrototypeOf(objects)){objects=[objects];}
resolution=resolution!==undefined?resolution:1;getCollisionPosition=getCollisionPosition!==undefined?getCollisionPosition:false;checkBoundingBox=checkBoundingBox!==undefined?checkBoundingBox:true;var canvas,canvasSize,ctx,obj,bitmap,i,data,length,pixel,pxArr,x,y,sumX,sumY,avX,avY,avDist,avDir;if(this.bmSize===0){return false;}
if(checkBoundingBox){objects=this.boundingBoxCollidesWith(objects,true);if(objects===false){return false;}}
