...
Input.Pointer::import ###*
@lends Input.Pointer.prototype
###
  ...
  checkPointer: (pointers, state) ->
    ...
    throw new Error("Argument state must be one of the following values: \"pressed\", \"released\" or \"down\"")  if [ #dev
      "pressed"
      "released"
      "down"
    ].indexOf(state) is -1
    ...
    while i < pointers.length
      ...
      switch state
        when "pressed"
          ret.push pointer  if pointer.events[0] > engine.last or pointer.events[1] > engine.last
        when "released"
          ret.push pointer  if -pointer.events[0] > engine.last or -pointer.events[1] > engine.last
        when "down"
