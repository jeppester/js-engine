...
Engine::import
  ...
  load: ->
    ...
    loader.loadClasses [@gameClassPath]
    @gameClassName = @gameClassPath.match(/(\w*)\.\w+$/)[1]
  ...
  initialize: ->
    ...
    # Create game object
    if window[@gameClassName] isnt "undefined"
      objectName = @gameClassName.substr(0, 1).toLowerCase() + @gameClassName.substr(1)
      eval objectName + " = new " + @gameClassName + "()"
    else
  ...
  convertSpeed: (speed, from, to) ->
    ...
    switch from
      ...
      when SPEED_PIXELS_PER_FRAMEswitch to
      when SPEED_PIXELS_PER_SECOND
        speed = speed / @gameTimeIncrease * 1000
      when SPEED_PIXELS_PER_FRAMEspeed
