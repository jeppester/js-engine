...
###
Engine.CustomLoop = (framesPerExecution, maskFunction) ->
  @framesPerExecution = (if framesPerExecution is `undefined` then 1 else framesPerExecution)
  @maskFunction = (if maskFunction is `undefined` then ->
    true
   else maskFunction)
  ...
  @last = (if engine.now then engine.now else new Date().getTime())
  @time = 0
  @execTime = 0
  return

Engine.CustomLoop::import ###*
@lends Engine.CustomLoop.prototype
###
