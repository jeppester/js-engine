...
###
  ...
  getBoundingRectangle: ->
    ...
    while i < @points.length
      ...
      i++
    new Math.Rectangle().setFromVectors startVector, endVector.subtract(startVector)
  ...
  contains: (object) ->
    if object instanceof Math.Vector
      @intersects(new Math.Line().setFromCoordinates(-123456, -98765, object.x, object.y), true) % 2
    else if object instanceof Math.Line
      not @intersects(object) and @contains(object.a)
    else if object instanceof Math.Circle
      
      # Check that the circle's center is placed inside the Polygon
      if @contains(new Math.Vector(object.x, object.y))
