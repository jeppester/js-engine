...
Main = (function() {
  ...
  Main.prototype.addObjects = function(count) {
    var col, cols, direction, directionInt, i, j, ref, results, row, rows, text, x, xInt, y, yInt;
    if (count == null) {
      count = 10;
    }
    ...
    for (i = j = 0, ref = count; 0 <= ref ? j < ref : j > ref; i = 0 <= ref ? ++j : --j) {
      x = col * xInt;
      y = row * yInt;
      text = new Engine.Views.TextBlock('TEXT', x, y, 100, {
        direction: direction,
        color: '#F00'
      });
      engine.currentRoom.addChildren(text);
      global.text = text;
