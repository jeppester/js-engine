...
c = class Polygon
  ...
  setFromCoordinates: (x1, y1, x2, y2, x3, y3) ->
    ...
    while i < numPoints
      ...
      @points.push new Geometry.Vector(x, y)
      i++
    this
  ...
  ###
  add: (vector) ->
    throw new Error("Argument vector should be of type Vector") if not vector instanceof Geometry.Vector #dev
    i = 0
  ...
  getBoundingRectangle: ->
    ...
    startVector = new Geometry.Vector(@points[0].x, @points[0].y)
    ...
    while i < @points.length
      ...
      i++
    new Geometry.Rectangle().setFromVectors startVector, endVector.subtract(startVector)
  ...
  contains: (object) ->
    if object instanceof Geometry.Vector
      @intersects(new Geometry.Line().setFromCoordinates(-123456, -98765, object.x, object.y), true) % 2
    else if object instanceof Geometry.Line
      not @intersects(object) and @contains(object.a)
    else if object instanceof Geometry.Circle

      # Check that the circle's center is placed inside the Polygon
      if @contains(new Geometry.Vector(object.x, object.y))
