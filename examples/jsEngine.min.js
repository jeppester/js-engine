...
loader=new Loader();loader.loadClasses([this.enginePath+'/classes/Animatable.js',this.enginePath+'/classes/View.js',this.enginePath+'/classes/Room.js',this.enginePath+'/classes/Vector.js',this.enginePath+'/classes/Line.js',this.enginePath+'/classes/Polygon.js',this.enginePath+'/classes/Rectangle.js',this.enginePath+'/classes/Circle.js',this.enginePath+'/classes/Camera.js',this.enginePath+'/classes/CustomLoop.js',this.enginePath+'/classes/Sprite.js',this.enginePath+'/classes/Collidable.js',this.enginePath+'/classes/TextBlock.js',this.enginePath+'/classes/GameObject.js',this.enginePath+'/classes/Keyboard.js',this.enginePath+'/classes/Pointer.js',this.enginePath+'/classes/Sound.js',this.enginePath+'/classes/Music.js']);gc=this.gameClassPath;loader.loadClasses([gc]);this.gameClassName=gc.match(/(\w*)\.\w+$/)[1];this.defaultTheme=this.options.themes[0];loader.onthemesloaded=function(){engine.initialize();};loader.loadThemes(this.options.themes);};Engine.prototype.initialize=function(){var i,d,objectName;this.objectIndex={};this.frames=0;this.last=new Date().getTime();this.now=this.last;this.gameTime=0;this.currentId=0;this.drawing=0;this.fps=0;this.fpsCounter=0;this.fpsSecCounter=0;this.roomList=[];this.masterRoom=new Room('master');this.currentRoom=new Room('main');this.defaultAnimationLoop=this.masterRoom.loops.eachFrame;this.defaultActivityLoop=this.masterRoom.loops.eachFrame;this.cameras.push(new Camera(new Rectangle(0,0,this.canvasResX,this.canvasResY),new Rectangle(0,0,this.canvasResX,this.canvasResY)));if(this.disableRightClick){this.arena.oncontextmenu=function(){return false;};}
...
return animations;};Animatable.prototype.stopAnimations=function(){var animations,roomId,room,name;animations=[];for(roomId=0;roomId<engine.roomList.length;roomId++){room=engine.roomList[roomId];for(name in room.loops){if(room.loops.hasOwnProperty(name)){room.loops[name].removeAnimationsOfObject(this);}}}};NewClass('View');View.prototype.View=function(){this.children=[];this.drawCacheCanvas=document.createElement('canvas');this.drawCacheCtx=this.drawCacheCanvas.getContext('2d');this.drawCacheEnabled=false;this.drawCacheOffset=new Vector();};View.prototype.addChildren=function(child1,child2){if(arguments.length===0){throw new Error('This function needs at least one argument');}
...
this.drawCacheEnabled=enabled;};View.prototype.cacheDrawing=function(){var drawRegion;drawRegion=this.getCombinedRedrawRegion();if(drawRegion){this.drawCacheOffset=new Vector(drawRegion.x,drawRegion.y);this.drawCacheCanvas.width=drawRegion.width;this.drawCacheCanvas.height=drawRegion.height;this.drawChildren(this.drawCacheCtx,this.drawCacheOffset.copy(),true);}};View.prototype.getCombinedRedrawRegion=function(){var box,addBox,i;if(this.getRedrawRegion){box=this.getRedrawRegion();}
...
i--;}
return removed;};View.prototype.removeAllChildren=function(purge){purge=purge!==undefined?purge:true;var rmChild;rmChild=this.children.splice(0,this.children.length);rmChild.forEach(function(){this.parent=undefined;if(purge){this.remove(purge);}});};View.prototype.drawChildren=function(c,cameraOffset){var i,child;if(this.drawCacheEnabled){c.save();c.drawImage(this.drawCacheCanvas,this.drawCacheOffset.x,this.drawCacheOffset.y,this.drawCacheCanvas.width,this.drawCacheCanvas.height);c.restore();}
else{for(i=0;i<this.children.length;i++){child=this.children[i];if(child.drawCanvas){child.drawCanvas(c,cameraOffset);if(engine.drawBoundingBoxes&&child.drawBoundingBox){child.drawBoundingBox(c,cameraOffset);}
if(engine.drawMasks&&child.drawMask){child.drawMask(c,cameraOffset);}}
if(child.drawChildren){child.drawChildren(c,cameraOffset);}}}};NewClass('Room',[View]);Room.prototype.Room=function(name,onEntered,onLeft){this.View();this.name=name?name:engine.roomList.length;this.onEntere=onEntered!==undefined?onEntered:function(){};this.onLeft=onLeft!==undefined?onLeft:function(){};this.loops={};this.addLoop('eachFrame',new CustomLoop());engine.addRoom(this);};Room.prototype.update=function(){var i;for(i in this.loops){if(this.loops.hasOwnProperty(i)){this.loops[i].execute();}}};Room.prototype.addLoop=function(name,loop){if(loop===undefined){throw new Error('Missing argument: loop');}
...
delete this.loops[name];};Room.prototype.remove=undefined;NewClass('Vector',[Animatable]);Vector.prototype.Vector=function(x,y){this.set(x,y);};Vector.prototype.set=function(x,y){this.x=x!==undefined?x:0;this.y=y!==undefined?y:0;return this;};Vector.prototype.setFromDirection=function(direction,length){if(typeof direction!=='number'){throw new Error('Argument direction should be of type: Number');}
...
this.width*=factor;this.height*=factor;return this;};Rectangle.prototype.combine=function(rectangle){if(!rectangle.implements(Rectangle)){throw new Error('Argument rectangle should be of type Rectangle');}
...
if(dist<0){break;}}
return dist;}
else{throw new Error('Agument object should be of type: Vector, Line, Circle, Rectangle or Polygon');}};Polygon.prototype.contains=function(object){if(object.implements(Vector)){if(this.intersects(new Line().setFromCoordinates(-100000,-100000,object.x,object.y),true)%2){return true;}
else{return false;}}
...
this.functionsQueue.push({object:caller,activity:func});};CustomLoop.prototype.detachFunction=function(caller,func){if(caller===undefined){throw new Error('Missing argument: caller');}
if(func===undefined){throw new Error('Missing argument: func');}
var removeArray,i,a;removeArray=[];for(i=0;i<this.functions.length;i++){a=this.functions[i];if(a.object===caller&&a.activity===func){removeArray.push(this.functions.splice(i,1));}}
for(i=0;i<this.functionsQueue.length;i++){a=this.functionsQueue[i];if(a.object===caller&&a.activity===func){removeArray.push(this.functionsQueue.splice(i,1));}}
if(removeArray.length){return removeArray;}
else{return false;}};CustomLoop.prototype.addFunctionsQueue=function(){this.functions=this.functions.concat(this.functionsQueue);this.functionsQueue=[];};CustomLoop.prototype.detachFunctionsByCaller=function(caller){if(caller===undefined){throw new Error('Missing argument: caller');}
var i;i=this.functions.length;while(i--){if(caller===this.functions[i].object){this.functions.splice(i,1);}}
i=this.functionsQueue.length;while(i--){if(caller===this.functionsQueue[i].object){this.functionsQueue.splice(i,1);}}};CustomLoop.prototype.schedule=function(func,delay,caller){if(func===undefined){throw new Error('Missing argument: function');}
if(delay===undefined){throw new Error('Missing argument: delay');}
caller=caller!==undefined?caller:this;this.executionsQueue.push({func:func,execTime:this.time+delay,caller:caller,});};CustomLoop.prototype.addExecutionsQueue=function(){this.executions=this.executions.concat(this.executionsQueue);this.executionsQueue=[];};CustomLoop.prototype.unSchedule=function(func,caller){if(func===undefined){throw new Error('Missing argument: function');}
caller=caller!==undefined?caller:this;var i,exec;for(i=0;i<this.executions.length;i++){exec=this.executions[i];if(caller===exec.caller&&(exec.func===func||exec.func.toString()===func)){this.executions.splice(i,1);break;}}
for(i=0;i<this.executionsQueue.length;i++){exec=this.executionsQueue[i];if(caller===exec.caller&&(exec.func===func||exec.func.toString()===func)){this.executionsQueue.splice(i,1);break;}}};CustomLoop.prototype.unScheduleByCaller=function(caller){if(caller===undefined){throw new Error('Missing argument: caller');}
for(i=0;i<this.executions.length;i++){exec=this.executions[i];if(caller===exec.caller){this.executions.splice(i,1);break;}}
for(i=0;i<this.executionsQueue.length;i++){exec=this.executionsQueue[i];if(caller===exec.caller){this.executionsQueue.splice(i,1);break;}}};CustomLoop.prototype.unScheduleAll=function(){this.executions=[];this.executionsQueue=[];};CustomLoop.prototype.addAnimation=function(animation){if(animation===undefined){throw new Error('Missing argument: animation');}
...
this.addFunctionsQueue();this.addExecutionsQueue();this.execTime=(new Date().getTime())-timer;};NewClass('Sprite',[View,Animatable,Vector]);Sprite.prototype.Sprite=function(source,x,y,dir,additionalProperties){if(source===undefined){throw new Error('Missing argument: source');}
this.View();this.Vector(x,y);this.source=source;this.dir=dir!==undefined?dir:0;engine.registerObject(this);this.imageNumber=0;this.imageLength=1;this.animationSpeed=30;this.animationLastSwitch=engine.gameTime;this.animationLoops=true;this.size=1;this.opacity=1;this.composite='source-over';this.importProperties(additionalProperties);if(!this.refreshSource()){throw new Error('Sprite source was not successfully loaded: '+source);}
...
this.source=source;this.refreshSource();};Sprite.prototype.drawCanvas=function(c,cameraOffset){var x,y;if(engine.avoidSubPixelRendering){x=Math.round(this.x-cameraOffset.x);y=Math.round(this.y-cameraOffset.y);}
...
if(this.dir!==0){c.translate(x,y);c.rotate(this.dir);c.drawImage(this.bm,(this.width+this.bm.spacing)*this.imageNumber,0,this.width,this.height,-this.offset.x*this.size,-this.offset.y*this.size,this.width*this.size,this.height*this.size);c.rotate(-this.dir);c.translate(-x,-y);}
else{c.drawImage(this.bm,(this.width+this.bm.spacing)*this.imageNumber,0,this.width,this.height,x-this.offset.x*this.size,y-this.offset.y*this.size,this.width*this.size,this.height*this.size);}
if(this.composite!=='source-over'){c.globalCompositeOperation='source-over';}
c.globalAlpha=1;};Sprite.prototype.getRedrawRegion=function(){var ret;ret=new Rectangle(-this.offset.x,-this.offset.y,Math.floor(this.bm.width/this.imageLength),this.bm.height);ret=ret.getPolygon();ret=ret.scale(this.size);ret=ret.rotate(this.dir);ret=ret.getBoundingRectangle().move(this.x,this.y);ret.x=Math.floor(ret.x-1);ret.y=Math.floor(ret.y-1);ret.width=Math.ceil(ret.width+2);ret.height=Math.ceil(ret.height+2);return ret;};NewClass('Collidable',[Sprite]);Collidable.prototype.Collidable=function(source,x,y,dir,additionalProperties){this.Sprite(source,x,y,dir,additionalProperties);this.mask=this.mask?this.mask:loader.getMask(source,this.getTheme());this.collisionResolution=this.collisionResolution?this.collisionResolution:engine.defaultCollisionResolution;};Collidable.prototype.boundingBoxCollidesWith=function(objects,getCollidingObjects){if(objects===undefined){throw new Error('Missing argument: objects');}
...
var canvas,mask,ctx,obj,bitmap,i,data,length,pixel,pxArr,x,y,avX,avY,avDist,avDir;if(!Array.prototype.isPrototypeOf(objects)){objects=[objects];}
getCollisionPosition=getCollisionPosition!==undefined?getCollisionPosition:false;mask=loader.getMask(this.source,this.getTheme());canvas=document.createElement('canvas');canvas.width=Math.ceil(mask.width*this.size);canvas.height=Math.ceil(mask.height*this.size);ctx=canvas.getContext('2d');ctx.fillStyle="#FFF";ctx.fillRect(0,0,canvas.width,canvas.height);ctx.globalAlpha=0.5;ctx.save();ctx.drawImage(mask,(this.width+this.bm.spacing)*this.imageNumber,0,this.width,this.height,0,0,this.width*this.size,this.height*this.size);ctx.translate(this.offset.x*this.size,this.offset.y*this.size);ctx.rotate(-this.dir);for(i=0;i<objects.length;i++){obj=objects[i];if(obj===this){continue;}
mask=loader.getMask(obj.source,obj.getTheme());ctx.translate(obj.x-this.x,obj.y-this.y);ctx.rotate(obj.dir);ctx.drawImage(obj.mask,(obj.width+obj.bm.spacing)*obj.imageNumber,0,obj.width,obj.height,-obj.offset.x*obj.size,-obj.offset.y*obj.size,obj.width*obj.size,obj.height*obj.size);ctx.translate(this.x-obj.x,this.y-obj.y);ctx.rotate(-obj.dir);}
ctx.restore();bitmap=ctx.getImageData(0,0,canvas.width,canvas.height);data=bitmap.data;length=data.length/4;pxArr=[];for(pixel=0;pixel<length;pixel+=this.collisionResolution){if(data[pixel*4]<127){if(getCollisionPosition){y=Math.floor(pixel/bitmap.width);x=pixel-y*bitmap.width;pxArr.push({x:x,y:y});}
else{return true;}}}
...
getCollidingObjects=getCollidingObjects!==undefined?getCollidingObjects:false;if(this.size===0){return false;}
...
else{return false;}};Collidable.prototype.drawBoundingBox=function(c,cameraOffset){var pol;pol=this.mask.bBox.copy().move(this.mask.width/2-this.offset.x,this.mask.height/2-this.offset.y).rotate(this.dir).scale(this.size);c.save();c.strokeStyle="#0F0";c.translate(this.x-cameraOffset.x,this.y-cameraOffset.y);c.beginPath();c.moveTo(pol.points[0].x,pol.points[0].y);c.lineTo(pol.points[1].x,pol.points[1].y);c.lineTo(pol.points[2].x,pol.points[2].y);c.lineTo(pol.points[3].x,pol.points[3].y);c.closePath();c.stroke();c.restore();};Collidable.prototype.drawMask=function(c,cameraOffset){var mask;if(this.mask){mask=this.mask;}
else{return;}
c.save();c.translate(this.x-cameraOffset.x,this.y-cameraOffset.y);c.rotate(this.dir);try{c.drawImage(this.mask,(this.width+this.bm.spacing)*this.imageNumber,0,this.width,this.height,-this.offset.x*this.size,-this.offset.y*this.size,this.width*this.size,this.height*this.size);}catch(e){console.log(this.source);console.log(this.bm);engine.stopMainLoop();throw new Error(e);}
c.restore();};NewClass('TextBlock',[Animatable,View,Vector]);TextBlock.prototype.TextBlock=function(string,x,y,width,additionalProperties){if(string===undefined){throw new Error('Missing argument: string');}
if(width===undefined){throw new Error('Missing argument: width');}
this.View();this.Vector(x,y);this.width=width;this.font='normal 14px Verdana';this.alignment='left';this.offset=new Vector();this.color="#000000";this.opacity=1;this.size=1;this.dir=0;this.composite='source-over';this.importProperties(additionalProperties);this.lineHeight=this.lineHeight?this.lineHeight:this.font.match(/[0.0-9]+/)*1.25;this.lines=[];this.lineWidth=[];this.bm=document.createElement('canvas');this.bmCtx=this.bm.getContext('2d');this.bm.width=this.width;this.bm.height=1000;engine.registerObject(this);this.setString(string);this.cacheRendering();if(engine.avoidSubPixelRendering){this.offset.x=Math.round(this.offset.x);this.offset.y=Math.round(this.offset.y);}};TextBlock.prototype.setString=function(string){if(string===undefined){throw new Error('Missing argument: string');}
...
c.rotate(this.dir);c.globalAlpha=this.opacity;c.globalCompositeOperation=this.composite;c.drawImage(this.bm,-this.offset.x*this.size,-this.offset.y*this.size,this.bm.width*this.size,this.bm.height*this.size);c.restore();}
TextBlock.prototype.getRedrawRegion=function(){var ret;ret=new Rectangle(-this.offset.x,-this.offset.y,this.bm.width,this.bm.height);ret=ret.getPolygon();ret=ret.scale(this.size);ret=ret.rotate(this.dir);ret=ret.getBoundingRectangle().move(this.x,this.y);ret.x=Math.floor(ret.x-1);ret.y=Math.floor(ret.y-1);ret.width=Math.ceil(ret.width+2);ret.height=Math.ceil(ret.height+2);return ret;};NewClass('GameObject',[Collidable]);GameObject.prototype.GameObject=function(source,x,y,dir,additionalProperties){if(source===undefined){throw new Error('Missing argument: source');}
...
break;}}
return ret.length?ret:false;};Pointer.prototype.findTouchNumber=function(){var i;for(i=0;i<this.touches.length;i++){if(!this.checkPointer(this.touches[i],'down')){return i;}}};Pointer.prototype.shapeIsHovered=function(shape,outside){if(!outside&&(shape.contains(this.mouse))){return true;}
...
return true;};Pointer.prototype.outside=function(){return this.mouse.x<0||this.mouse.x>engine.arena.offsetWidth||this.mouse.y<0||this.mouse.y>engine.arena.offsetHeight;};NewClass('Sound');Sound.prototype.Sound=function(audioElement){if(audioElement===undefined){throw new Error('Missing argument: audioElement');}
