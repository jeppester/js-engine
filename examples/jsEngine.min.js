...
eval('window.'+className+' = function () {this.'+className+'.apply(this, arguments); }');window[className].prototype[className]=function(){};newClass=window[className];newClass.prototype.className=className;newClass.prototype.extendedClasses=[];newClass.prototype.extends=function(checkClass){return this.extendedClasses.indexOf(checkClass)!==-1;};newClass.prototype.implements=function(checkClass){return(checkClass.prototype.isPrototypeOf(this)?true:this.extends(checkClass));};function inherit(newClass,inheritClass){var functionName;newClass.prototype.extendedClasses.push(inheritClass);Array.prototype.push.apply(newClass.prototype.extendedClasses,inheritClass.prototype.extendedClasses);for(functionName in inheritClass.prototype){if(typeof inheritClass.prototype[functionName]==="function"){newClass.prototype[functionName]=inheritClass.prototype[functionName];}}}
...
else{console.log('XMLHttpRequest failed: '+url);}}};Engine.prototype.purge=function(obj){var name,loop,i;if(obj===undefined){throw new Error(obj);}
...
else{d.drawChildren(d.ctx);}}};Engine.prototype.dumpScreen=function(){var dataString,img;dataString=this.mainCanvas.toDataURL().replace(/image\/png/,'image/octet-stream');a=document.createElement('a');a.href=dataString;a.setAttribute('download','screendump.png');document.body.appendChild(a);a.click();document.body.removeChild(a,document.body);}
NewClass('Animator');Animator.prototype.addAnimation=function(animation,loop){if(animation===undefined){throw new Error('Missing argument: animation');}
