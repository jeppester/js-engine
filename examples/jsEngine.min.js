...
return speed;};Engine.prototype.setSoundsMuted=function(muted){muted=muted!==undefined?muted:true;if(muted){loader.getAllSounds().forEach(function(){this.stopAll();});}
this.soundsMuted=muted;};Engine.prototype.goToRoom=function(room){if(room===undefined){throw new Error('Missing argument: room');}
...
this.currentRoom.onLeave();this.currentRoom=room;this.currentRoom.onEnter();}
Engine.prototype.addRoom=function(room){if(room===undefined){throw new Error('Missing argument: room');}
...
Engine.prototype.removeRoom=function(room){if(room===undefined){throw new Error('Missing argument: room');}
...
Engine.prototype.setMusicMuted=function(muted){muted=muted!==undefined?muted:true;if(muted){loader.getAllMusic().forEach(function(){this.stop();});}
...
else{throw new Error('XMLHttpRequest failed: '+url);}}};Engine.prototype.purge=function(obj){var name,loop,roomId,room,i;if(obj===undefined){throw new Error(obj);}
if(typeof obj==="string"){obj=this.objectIndex[obj];}
for(roomId=0;roomId<this.roomList.length;roomId++){room=this.roomList[roomId];for(name in room.loops){if(room.loops.hasOwnProperty(name)){loop=room.loops[name];loop.detachFunctionsByCaller(obj);loop.removeAnimationsOfObject(obj);loop.unScheduleByCaller(obj);}}}
if(obj.parent){obj.parent.removeChildren(obj);}
delete this.objectIndex[obj.id];};Engine.prototype.redraw=function(){var i;for(var i=0;i<this.cameras.length;i++){this.cameras[i].capture();this.cameras[i].draw(this.mainCanvas.getContext('2d'));}};Engine.prototype.dumpScreen=function(){var dataString,a;dataString=this.mainCanvas.toDataURL().replace(/image\/png/,'image/octet-stream');a=document.createElement('a');a.href=dataString;a.setAttribute('download','screendump.png');document.body.appendChild(a);a.click();document.body.removeChild(a,document.body);};Engine.prototype.ease=function(type,t,b,c,d){var a;switch(type){case"linear":t/=d;return b+c*t;case"quadIn":t/=d;return b+c*t*t;case"quadOut":t/=d;return b-c*t*(t-2);case"quadInOut":t=t/d*2;if(t<1){return b+c*t*t/2;}else{t--;return b+c*(1-t*(t-2))/2;}
...
i--;}
return removed;};View.prototype.drawChildren=function(c,cameraOffset,forceRedraw){var i;if(this.drawCacheEnabled&&!forceRedraw){c.save();c.drawImage(this.drawCacheCanvas,this.drawCacheOffset.x,this.drawCacheOffset.y,this.drawCacheCanvas.width,this.drawCacheCanvas.height);c.restore();}
else{if(this.drawCanvas){this.drawCanvas(c,cameraOffset);if(engine.drawBBoxes&&this.drawBBox){if(engine.useRotatedBoundingBoxes){this.drawRotatedBBox(c,cameraOffset);}
else{this.drawBBox(c,cameraOffset);}}
if(engine.drawMasks&&this.drawMask){this.drawMask(c,cameraOffset);}}
for(i=0;i<this.children.length;i++){if(this.children[i].drawChildren){this.children[i].drawChildren(c,cameraOffset,forceRedraw);}
else if(this.children[i].drawCanvas){this.children[i].drawCanvas(c,cameraOffset,forceRedraw);}}}};NewClass('Room',[View]);Room.prototype.Room=function(name,onEntered,onLeft){this.View();this.name=name?name:engine.roomList.length;this.onEntere=onEntered!==undefined?onEntered:function(){};this.onLeft=onLeft!==undefined?onLeft:function(){};this.loops={};this.addLoop('eachFrame',new CustomLoop());engine.addRoom(this);};Room.prototype.update=function(){var i;for(i in this.loops){if(this.loops.hasOwnProperty(i)){this.loops[i].execute();}}};Room.prototype.addLoop=function(name,loop){if(loop===undefined){throw new Error('Missing argument: loop');}
...
var pol=this.getPolygon();pol.points=pol.points.concat(rectangle.getPolygon().points);return pol.getBoundingRectangle();}
Rectangle.prototype.getPolygon=function(){return new Polygon(this.getPoints());};Rectangle.prototype.getPoints=function(){return[new Vector(this.x,this.y),new Vector(this.x+this.width,this.y),new Vector(this.x+this.width,this.y+this.height),new Vector(this.x,this.y+this.height)];};Rectangle.prototype.getArea=function(){return this.width*this.height;};Rectangle.prototype.getDiagonal=function(){return Math.sqrt(Math.pow(this.width,2)+Math.pow(this.height,2));};Rectangle.prototype.getDistance=function(object){return this.getPolygon().getDistance(object);};Rectangle.prototype.contains=function(object){return this.getPolygon().contains(object);};Rectangle.prototype.intersects=function(object){return this.getPolygon().intersects(object);};Rectangle.prototype.drawCanvas=function(c,cameraOffset){c.save();c.translate(-cameraOffset.x,-cameraOffset.y);c.strokeStyle="#f00";c.beginPath();c.moveTo(this.x,this.y);c.lineTo(this.x+this.width,this.y);c.lineTo(this.x+this.width,this.y+this.height);c.lineTo(this.x,this.y+this.height);c.lineTo(this.x,this.y);c.stroke();c.closePath();c.restore();};NewClass('Polygon');Polygon.prototype.Polygon=function(points){this.setFromPoints(points);};Polygon.prototype.setFromPoints=function(points){this.points=points;return this;};Polygon.prototype.setFromCoordinates=function(x1,y1,x2,y2,x3,y3){var numPoints,i,x,y;numPoints=Math.floor(arguments.length/2);this.points=[];for(i=0;i<numPoints;i++){x=arguments[i*2];y=arguments[i*2+1];if(typeof x!=='number'||typeof y!=='number'){throw new Error('All arguments should be of type: Number');}
...
else{return false;}};CustomLoop.prototype.addQueue=function(){this.activities=this.activities.concat(this.activitiesQueue);this.activitiesQueue=[];};CustomLoop.prototype.detachFunctionsByCaller=function(caller){if(caller===undefined){throw new Error('Missing argument: caller');}
var i;i=this.activities.length;while(i--){if(caller===this.activities[i].object){this.activities.splice(i,1);}}
i=this.activitiesQueue.length;while(i--){if(caller===this.activitiesQueue[i].object){this.activitiesQueue.splice(i,1);}}};CustomLoop.prototype.schedule=function(func,delay,caller){if(func===undefined){throw new Error('Missing argument: function');}
...
caller=caller!==undefined?caller:this;var i,exec;for(i=0;i<this.scheduledExecutions.length;i++){exec=this.scheduledExecutions[i];if(caller===exec.caller&&(exec.func===func||exec.func.toString()===func)){this.scheduledExecutions.splice(i,1);break;}}};CustomLoop.prototype.unScheduleByCaller=function(caller){if(caller===undefined){throw new Error('Missing argument: caller');}
for(i=0;i<this.scheduledExecutions.length;i++){exec=this.scheduledExecutions[i];if(caller===exec.caller){this.scheduledExecutions.splice(i,1);break;}}};CustomLoop.prototype.addAnimation=function(animation){if(animation===undefined){throw new Error('Missing argument: animation');}
...
this.lastFrame=engine.frames;this.last=engine.now;this.updateAnimations();for(i=0;i<this.scheduledExecutions.length;i++){exec=this.scheduledExecutions[i];if(this.time>=exec.execTime){exec.func.call(exec.caller);this.scheduledExecutions.splice(i,1);}}
for(i=0;i<this.activities.length;i++){exec=this.activities[i];if(!exec.activity){throw new Error('Trying to exec non-existent attached function');return;}
exec.activity.call(exec.object);}
this.addQueue();this.execTime=(new Date().getTime())-timer;};NewClass('Sprite',[View,Animatable,Vector]);Sprite.prototype.Sprite=function(source,x,y,dir,additionalProperties){if(source===undefined){throw new Error('Missing argument: source');}
this.View();this.Vector(x,y);this.source=source;this.dir=dir!==undefined?dir:0;engine.registerObject(this);this.imageSpeed=10;this.currentImage=0;this.imageTimeOffset=engine.gameTime;this.size=1;this.opacity=1;this.composite='source-over';this.importProperties(additionalProperties);if(!this.refreshSource()){throw new Error('Sprite source was not successfully loaded: '+source);}
...
if(this.dir!==0){c.translate(x,y);c.rotate(this.dir);c.drawImage(this.bm,(this.width+this.bm.spacing)*this.currentImage,0,this.width,this.height,-this.offset.x*this.size,-this.offset.y*this.size,this.width*this.size,this.height*this.size);c.rotate(-this.dir);c.translate(-x,-y);}
else{c.drawImage(this.bm,(this.width+this.bm.spacing)*this.currentImage,0,this.width,this.height,x-this.offset.x*this.size,y-this.offset.y*this.size,this.width*this.size,this.height*this.size);}
if(this.composite!=='source-over'){c.globalCompositeOperation='source-over';}
c.globalAlpha=1;};Sprite.prototype.getRedrawRegion=function(){var ret;ret=new Rectangle(-this.offset.x,-this.offset.y,Math.floor(this.bm.width/this.imageLength),this.bm.height);ret=ret.getPolygon();ret=ret.scale(this.size);ret=ret.rotate(this.dir)
ret=ret.getBoundingRectangle().move(this.x,this.y);ret.x=Math.floor(ret.x-1);ret.y=Math.floor(ret.y-1);ret.width=Math.ceil(ret.width+2);ret.height=Math.ceil(ret.height+2);return ret;}
NewClass('Collidable',[Sprite]);Collidable.prototype.Collidable=function(source,x,y,dir,additionalProperties){this.Sprite(source,x,y,dir,additionalProperties);this.mask=this.mask?this.mask:loader.getMask(source,this.getTheme());this.collisionResolution=this.collisionResolution?this.collisionResolution:engine.defaultCollisionResolution;};Collidable.prototype.polygonCollidesWith=function(objects,getCollidingObjects){if(objects===undefined){throw new Error('Missing argument: objects');}
...
if(collidingObjects.length){return collidingObjects;}
else{return false;}};Collidable.prototype.collidesWith=function(objects,getCollisionPosition,checkBBox){if(objects===undefined){throw new Error('Missing argument: objects');}
if(!Array.prototype.isPrototypeOf(objects)){objects=[objects];}
getCollisionPosition=getCollisionPosition!==undefined?getCollisionPosition:false;checkBBox=checkBBox!==undefined?checkBBox:true;var canvas,mask,ctx,obj,bitmap,i,data,length,pixel,pxArr,x,y,avX,avY,avDist,avDir;if(this.size===0){return false;}
...
lt.parentNode.removeChild(lt);};TextBlock.prototype.drawCanvas=function(c,cameraOffset){if(/^\s*$/.test(this.string)){return;}
c.save();if(engine.avoidSubPixelRendering){c.translate(Math.round(this.x-cameraOffset.x),Math.round(this.y-cameraOffset.y));}
else{c.translate(this.x-cameraOffset.x,this.y-cameraOffset.y);}
c.rotate(this.dir);c.globalAlpha=this.opacity;c.globalCompositeOperation=this.composite;c.drawImage(this.bm,-this.offset.x*this.size,-this.offset.y*this.size,this.bm.width*this.size,this.bm.height*this.size);c.restore();};TextBlock.prototype.getRedrawRegion=function(){var ret;ret=new Rectangle(-this.offset.x,-this.offset.y,this.bm.width,this.bm.height);ret=ret.getPolygon();ret=ret.scale(this.size);ret=ret.rotate(this.dir)
ret=ret.getBoundingRectangle().move(this.x,this.y);ret.x=Math.floor(ret.x-1);ret.y=Math.floor(ret.y-1);ret.width=Math.ceil(ret.width+2);ret.height=Math.ceil(ret.height+2);return ret;}
NewClass('GameObject',[Collidable]);GameObject.prototype.GameObject=function(source,x,y,dir,additionalProperties){if(source===undefined){throw new Error('Missing argument: source');}
