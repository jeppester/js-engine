...
loader=new Loader();loader.loadClasses([this.enginePath+'/classes/Animatable.js',this.enginePath+'/classes/Animator.js',this.enginePath+'/classes/View.js',this.enginePath+'/classes/Vector.js',this.enginePath+'/classes/Line.js',this.enginePath+'/classes/Polygon.js',this.enginePath+'/classes/Rectangle.js',this.enginePath+'/classes/Circle.js',this.enginePath+'/classes/CustomLoop.js',this.enginePath+'/classes/Sprite.js',this.enginePath+'/classes/Collidable.js',this.enginePath+'/classes/TextBlock.js',this.enginePath+'/classes/GameObject.js',this.enginePath+'/classes/Keyboard.js',this.enginePath+'/classes/Pointer.js',this.enginePath+'/classes/Sound.js',this.enginePath+'/classes/Music.js']);gc=this.gameClassPath;loader.loadClasses([gc]);this.gameClassName=gc.match(/(\w*)\.\w+$/)[1];this.defaultTheme=this.options.themes[0];loader.onthemesloaded=function(){engine.initialize();};loader.loadThemes(this.options.themes);};Engine.prototype.initialize=function(){var i,d,objectName;this.objectIndex={};this.frames=0;this.last=new Date().getTime();this.now=this.last;this.gameTime=0;this.executingLoops=false;this.currentId=0;this.drawing=0;this.loops={};this.fps=0;this.fpsCounter=0;this.fpsSecCounter=0;this.depth=[];this.addLoop('eachFrame',new CustomLoop());this.defaultAnimationLoop='eachFrame';this.defaultActivityLoop='eachFrame';this.depthMap=[];this.options.depths=this.options.depths!==undefined?this.options.depths:1;for(i=0;i<this.options.depths;i++){d=new View();d.manualRedraw=this.manualRedrawDepths.indexOf(i)!==-1;d.composited=this.compositedDepths.indexOf(i)!==-1;d.ownCanvas=this.makeCanvas();if(d.manualRedraw||d.composited){d.ctx=d.ownCanvas.getContext('2d');}
...
else{throw new Error('XMLHttpRequest failed: '+url);}}};Engine.prototype.purge=function(obj){var name,loop,i;if(obj===undefined){throw new Error(obj);}
if(typeof obj==="string"){obj=this.objectIndex[obj];}
for(name in this.loops){if(this.loops.hasOwnProperty(name)){loop=this.loops[name];i=loop.activities.length;while(i--){if(obj===loop.activities[i].object){loop.activities.splice(i,1);}}
...
delete this.objectIndex[obj.id];};Engine.prototype.redraw=function(drawManualRedrawDepths){if(drawManualRedrawDepths===undefined){throw new Error('Missing argument: manualRedrawDepths');}
...
else{d.drawChildren(d.ctx);}}};Engine.prototype.dumpScreen=function(){var dataString,a;dataString=this.mainCanvas.toDataURL().replace(/image\/png/,'image/octet-stream');a=document.createElement('a');a.href=dataString;a.setAttribute('download','screendump.png');document.body.appendChild(a);a.click();document.body.removeChild(a,document.body);};NewClass('Animator');Animator.prototype.addAnimation=function(animation,loop){if(animation===undefined){throw new Error('Missing argument: animation');}
...
for(i=0;i<this.children.length;i++){if(this.children[i].drawChildren){this.children[i].drawChildren(ctx);}}};NewClass('Vector',[Animatable,View]);Vector.prototype.Vector=function(x,y){this.View();this.set(x,y);};Vector.prototype.set=function(x,y){this.x=x!==undefined?x:0;this.y=y!==undefined?y:0;return this;};Vector.prototype.setFromDirection=function(direction,length){if(typeof direction!=='number'){throw new Error('Argument direction should be of type: Number');}
...
else{throw new Error('Agument object should be of type: Vector, Line, Circle, Rectangle or Polygon');}};Line.prototype.drawCanvas=function(c){c.save();c.strokeStyle="#f00";c.beginPath();c.moveTo(this.a.x,this.a.y);c.lineTo(this.b.x,this.b.y);c.stroke();c.restore();};NewClass('Rectangle',[Animatable,View]);Rectangle.prototype.Rectangle=function(x,y,width,height){this.View();this.set(x,y,width,height);};Rectangle.prototype.set=function(x,y,width,height){this.x=x!==undefined?x:0;this.y=y!==undefined?y:0;this.width=width!==undefined?width:0;this.height=height!==undefined?height:0;return this;};Rectangle.prototype.setFromVectors=function(position,size){position=position!==undefined?position:new Vector();size=size!==undefined?size:new Vector();this.x=position.x;this.y=position.y;this.width=size.x;this.height=size.y;return this;};Rectangle.prototype.copy=function(){return new Rectangle(this.x,this.y,this.width,this.height);};Rectangle.prototype.move=function(x,y){if(typeof x!=='number'){throw new Error('Argument x should be of type: Number');}
if(typeof y!=='number'){throw new Error('Argument y should be of type: Number');}
...
this.width*=factor;this.height*=factor;return this;};Rectangle.prototype.getPolygon=function(){return new Polygon(this.getPoints());};Rectangle.prototype.getPoints=function(){return[new Vector(this.x,this.y),new Vector(this.x+this.width,this.y),new Vector(this.x+this.width,this.y+this.height),new Vector(this.x,this.y+this.height)];};Rectangle.prototype.getArea=function(){return this.width*this.height;};Rectangle.prototype.getDiagonal=function(){return Math.sqrt(Math.pow(this.width,2)+Math.pow(this.height,2));};Rectangle.prototype.getDistance=function(object){return this.getPolygon().getDistance(object);};Rectangle.prototype.contains=function(object){return this.getPolygon().contains(object);};Rectangle.prototype.intersects=function(object){return this.getPolygon().intersects(object);};Rectangle.prototype.drawCanvas=function(c){c.save();c.strokeStyle="#f00";c.beginPath();c.moveTo(this.x,this.y);c.lineTo(this.x+this.width,this.y);c.lineTo(this.x+this.width,this.y+this.height);c.lineTo(this.x,this.y+this.height);c.lineTo(this.x,this.y);c.stroke();c.closePath();c.restore();};NewClass('Polygon',[View]);Polygon.prototype.Polygon=function(points){this.View();this.setFromPoints(points);};Polygon.prototype.setFromPoints=function(points){this.points=points;return this;};Polygon.prototype.setFromCoordinates=function(x1,y1,x2,y2,x3,y3){var numPoints,i,x,y;numPoints=Math.floor(arguments.length/2);this.points=[];for(i=0;i<numPoints;i++){x=arguments[i*2];y=arguments[i*2+1];if(typeof x!=='number'||typeof y!=='number'){throw new Error('All arguments should be of type: Number');}
...
return lines;};Polygon.prototype.getDistance=function(object){var dist,lines,objLines,i,ii,pVector;dist=2E+10308;lines=this.getLines();if(object.implements(Vector)){for(i=0;i<lines.length;i++){dist=Math.min(dist,lines[i].getDistance(object));if(dist<0){break;}}
return dist;}
...
else{return false;}};Polygon.prototype.drawCanvas=function(c){var i,len;c.save();c.strokeStyle="#f00";c.beginPath();len=this.points.length;c.moveTo(this.points[len-1].x,this.points[len-1].y);for(i=0;i<len;i++){c.lineTo(this.points[i].x,this.points[i].y);}
c.stroke();c.restore();};NewClass('Circle',[Animatable,View]);Circle.prototype.Circle=function(x,y,radius){this.View();this.set(x,y,radius);};Circle.prototype.set=function(x,y,radius){x=x!==undefined?x:0;y=y!==undefined?y:0;radius=radius!==undefined?radius:0;this.x=x;this.y=y;this.radius=radius;return this;};Circle.prototype.copy=function(){return new Circle(this.x,this.y,this.radius);};Circle.prototype.move=function(x,y){if(typeof x!=='number'){throw new Error('Argument x should be of type: Number');}
if(typeof y!=='number'){throw new Error('Argument y should be of type: Number');}
...
c.globalAlpha=this.opacity;c.rotate(this.dir);c.globalCompositeOperation=this.composite;c.drawImage(this.bm,(this.width+this.bm.spacing)*this.currentImage,0,this.width,this.height,-this.offset.x*this.size,-this.offset.y*this.size,this.width*this.size,this.height*this.size);c.restore();};NewClass('Collidable',[Sprite]);Collidable.prototype.Collidable=function(source,x,y,dir,additionalProperties){this.Sprite(source,x,y,dir,additionalProperties);};Collidable.prototype.polygonCollidesWith=function(objects,getCollidingObjects){if(objects===undefined){throw new Error('Missing argument: objects');}
...
c.rotate(this.dir);c.globalAlpha=this.opacity;c.globalCompositeOperation=this.composite;c.drawImage(this.cache,-this.offset.x*this.size,-this.offset.y*this.size,this.cache.width*this.size,this.cache.height*this.size);c.restore();};NewClass('GameObject',[Collidable]);GameObject.prototype.GameObject=function(source,x,y,dir,additionalProperties){if(source===undefined){throw new Error('Missing argument: source');}
if(x===undefined){throw new Error('Missing argument: x');}
if(y===undefined){throw new Error('Missing argument: y');}
this.Collidable(source,x,y,dir,additionalProperties);this.mask=this.mask?this.mask:loader.getMask(source,this.getTheme());this.loop=this.loop?this.loop:'eachFrame';engine.loops[this.loop].attachFunction(this,this.updatePosition);this.speed=this.speed?this.speed:new Vector(0,0);this.alive=true;};GameObject.prototype.updatePosition=function(){if(this.alive){this.add(engine.convertSpeed(this.speed));}};NewClass('Keyboard');Keyboard.prototype.Keyboard=function(){var key;document.addEventListener('keydown',function(event){keyboard.onKeyDown(event);},false);document.addEventListener('keyup',function(event){keyboard.onKeyUp(event);},false);this.keys=new Array(200);for(key=0;key<this.keys.length;key++){this.keys[key]={events:[]};}};Keyboard.prototype.onKeyDown=function(event){var key;if(event===undefined){throw new Error('Missing argument: event');}
