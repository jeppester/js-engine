...
var cp;cp=[];cp.push.apply(cp,this);for(var i=0;i<cp.length;i++){func.call(cp[i],i);}};Object.prototype.importProperties=function(from){var i;for(i in from){if(from.hasOwnProperty(i)){if(i===undefined){continue;}
this[i]=from[i];}}};Object.prototype.implements=function(checkClass){return(checkClass.prototype.isPrototypeOf(this)?true:this.inherits(checkClass));};Object.prototype.inherits=function(checkClass){if(this.inheritedClasses){return this.inheritedClasses.indexOf(checkClass)!==-1;}
...
NewClass=function(className,inherits){var i,inheritClass,newClass;if(!/^\w*$/.test(className)){throw new Error("Invalid class name: "+className);}
...
this.running=false;this.canvasResX=800;this.canvasResY=600;this.enginePath='js/jsEngine';this.themesPath='themes';this.drawBoundingBoxes=false;this.drawMasks=false;this.pauseOnBlur=true;this.disableRightClick=true;this.arena=document.getElementById('arena');this.autoResize=true;this.autoResizeLimitToResolution=true;this.cachedSoundCopies=5;this.gameClassPath="js/Game.js";this.loadText="jsEngine loading...";this.backgroundColor="#FFF";this.timeFactor=1;this.disableTouchScroll=true;this.cameras=[];this.defaultCollisionResolution=5;this.soundsMuted=false;this.musicMuted=false;copyOpt=['backgroundColor','disableTouchScroll','defaultCollisionResolution','loadText','soundsMuted','musicMuted','cachedSoundCopies','avoidSubPixelRendering','arena','disableRightClick','pauseOnBlur','drawBoundingBoxes','drawMasks','canvasResX','canvasResY','autoResize','autoResizeLimitToResolution','enginePath','themesPath','gameClassPath'];for(i=0;i<copyOpt.length;i++){opt=copyOpt[i];if(this.options[opt]!==undefined){this[opt]=this.options[opt];delete this.options[opt];}}
...
loader=new Loader();loader.loadClasses([this.enginePath+'/classes/Animatable.js',this.enginePath+'/classes/Vector.js',this.enginePath+'/classes/View.js',this.enginePath+'/classes/Room.js',this.enginePath+'/classes/Line.js',this.enginePath+'/classes/Polygon.js',this.enginePath+'/classes/Rectangle.js',this.enginePath+'/classes/Circle.js',this.enginePath+'/classes/Camera.js',this.enginePath+'/classes/CustomLoop.js',this.enginePath+'/classes/Sprite.js',this.enginePath+'/classes/Collidable.js',this.enginePath+'/classes/TextBlock.js',this.enginePath+'/classes/GameObject.js',this.enginePath+'/classes/Keyboard.js',this.enginePath+'/classes/Pointer.js',this.enginePath+'/classes/Sound.js',this.enginePath+'/classes/Music.js']);gc=this.gameClassPath;loader.loadClasses([gc]);this.gameClassName=gc.match(/(\w*)\.\w+$/)[1];this.defaultTheme=this.options.themes[0];loader.onthemesloaded=function(){engine.initialize();};loader.loadThemes(this.options.themes);};Engine.prototype.initialize=function(){var i,d,objectName;this.objectIndex={};this.frames=0;this.last=new Date().getTime();this.now=this.last;this.gameTime=0;this.currentId=0;this.drawing=0;this.fps=0;this.fpsCounter=0;this.fpsSecCounter=0;this.roomList=[];this.masterRoom=new Room('master');this.currentRoom=new Room('main');this.defaultAnimationLoop=this.masterRoom.loops.eachFrame;this.defaultActivityLoop=this.masterRoom.loops.eachFrame;this.cameras.push(new Camera(new Rectangle(0,0,this.canvasResX,this.canvasResY),new Rectangle(0,0,this.canvasResX,this.canvasResY)));if(this.disableRightClick){this.arena.oncontextmenu=function(){return false;};}
...
this.startMainLoop();console.log('jsEngine started');if(this.onStarted){this.onStarted();}};Engine.prototype.setAutoResize=function(enable){if(enable&&!this.autoResize){this.autoResize=true;this.autoResizeCanvas();window.addEventListener('resize',engine.autoResizeCanvas,false);window.addEventListener('load',engine.autoResizeCanvas,false);}
else if(!enable&&this.autoResize){this.autoResize=false;window.removeEventListener('resize',engine.autoResizeCanvas,false);window.removeEventListener('load',engine.autoResizeCanvas,false);this.arena.style.top="50%";this.arena.style.left="50%";this.arena.style.marginLeft=-this.canvasResX/2+"px";this.arena.style.marginTop=-this.canvasResY/2+"px";this.mainCanvas.style.width=this.canvasResX+"px";this.mainCanvas.style.height=this.canvasResY+"px";}};Engine.prototype.autoResizeCanvas=function(){if(this!==engine){engine.autoResizeCanvas();return;}
...
this.arena.style.top="50%";this.arena.style.left="50%";this.arena.style.marginTop=-h/2+"px";this.arena.style.marginLeft=-w/2+"px";this.mainCanvas.style.height=h+"px";this.mainCanvas.style.width=w+"px";};Engine.prototype.convertSpeed=function(speed,from,to){if(speed===undefined){throw new Error('Missing argument: speed');}
...
return speed;};Engine.prototype.goToRoom=function(room){if(room===undefined){throw new Error('Missing argument: room');}
...
this.currentRoom.onLeave();this.currentRoom=room;this.currentRoom.onEnter();};Engine.prototype.addRoom=function(room){if(room===undefined){throw new Error('Missing argument: room');}
...
this.roomList.push(room);};Engine.prototype.removeRoom=function(room){if(room===undefined){throw new Error('Missing argument: room');}
...
this.roomList.splice(i,1);};Engine.prototype.setSoundsMuted=function(muted){muted=muted!==undefined?muted:true;if(muted){loader.getAllSounds().forEach(function(){this.stopAll();});}
...
var i,refreshSource;enforce=enforce!==undefined?enforce:false;this.defaultTheme=themeName;refreshSource=function(){if(this.refreshSource){this.refreshSource();}};this.currentRoom.setTheme(undefined,enforce);};Engine.prototype.startMainLoop=function(){if(this.running){return;}
this.now=new Date().getTime();this.running=true;engine.mainLoop();};Engine.prototype.stopMainLoop=function(){if(!this.running){return;}
this.running=false;};Engine.prototype.mainLoop=function(){var name;if(!this.running){return;}
...
this.objectIndex[id]=obj;obj.id=id;return id;};Engine.prototype.loadFiles=function(filePaths){var i,req,codeString;if(typeof filePaths==="string"){filePaths=[filePaths];}
...
window.loadedFiles=window.loadedFiles.concat(filePaths);};Engine.prototype.ajaxRequest=function(url,params,async,callback,caller){if(url===undefined){throw new Error('Missing argument: url');}
...
req.open('POST',url,async);req.setRequestHeader("Content-type","application/x-www-form-urlencoded");req.send(params);if(!async){if(req.readyState===4&&req.status===200){callback(req.responseText);}
else{throw new Error('XMLHttpRequest failed: '+url);}}};Engine.prototype.purge=function(obj){var len,name,loop,roomId,room,i;if(obj===undefined){throw new Error(obj);}
...
delete this.objectIndex[obj.id];};Engine.prototype.redraw=function(){var i;for(i=0;i<this.cameras.length;i++){this.cameras[i].capture();this.cameras[i].draw(this.mainCanvas.getContext('2d'));}};Engine.prototype.dumpScreen=function(){var dataString,a;dataString=this.mainCanvas.toDataURL().replace(/image\/png/,'image/octet-stream');a=document.createElement('a');a.href=dataString;a.setAttribute('download','screendump.png');document.body.appendChild(a);a.click();document.body.removeChild(a,document.body);};Engine.prototype.ease=function(type,t,b,c,d){var a;switch(type){case"linear":t/=d;return b+c*t;case"quadIn":t/=d;return b+c*t*t;case"quadOut":t/=d;return b-c*t*(t-2);case"quadInOut":t=t/d*2;if(t<1){return b+c*t*t/2;}else{t--;return b+c*(1-t*(t-2))/2;}
...
return b+c;};NewClass('Animatable');Animatable.prototype.animate=function(properties,options){if(properties===undefined){throw new Error('Missing argument: properties');}
...
loop.addAnimation(anim);};Animatable.prototype.isAnimated=function(){var roomId,room,name,loop,animId,animation;for(roomId=0;roomId<engine.roomList.length;roomId++){room=engine.roomList[roomId];for(name in room.loops){if(room.loops.hasOwnProperty(name)){loop=room.loops[name];for(animId=loop.animations.length-1;animId>-1;animId--){animation=loop.animations[animId];if(animation.obj===this){return true;}}}}}
return false;};Animatable.prototype.getAnimations=function(){var animations,roomId,room,name,loop,animId,animation;animations=[];for(roomId=0;roomId<engine.roomList.length;roomId++){room=engine.roomList[roomId];for(name in room.loops){if(room.loops.hasOwnProperty(name)){loop=room.loops[name];for(animId=loop.animations.length-1;animId>-1;animId--){animation=loop.animations[animId];if(animation.obj===this){animations.push(animation);}}}}}
return animations;};Animatable.prototype.stopAnimations=function(){var animations,roomId,room,name;animations=[];for(roomId=0;roomId<engine.roomList.length;roomId++){room=engine.roomList[roomId];for(name in room.loops){if(room.loops.hasOwnProperty(name)){room.loops[name].removeAnimationsOfObject(this);}}}};NewClass('Vector',[Animatable]);Vector.prototype.Vector=function(x,y){this.set(x,y);};Vector.prototype.set=function(x,y){this.x=x!==undefined?x:0;this.y=y!==undefined?y:0;return this;};Vector.prototype.setFromDirection=function(direction,length){if(typeof direction!=='number'){throw new Error('Argument direction should be of type: Number');}
...
this.setFromDirection(this.getDirection()+dir,this.getLength());return this;};Vector.prototype.scale=function(scaleH,scaleV){if(typeof scaleH!=='number'){throw new Error('Argument scaleH should be of type Number');}
scaleV=scaleV!==undefined?scaleV:scaleH;this.x*=scaleH;this.y*=scaleV;return this;};Vector.prototype.add=function(vector){if(vector.implements(Vector)){this.x+=vector.x;this.y+=vector.y;}
...
else{throw new Error('Agument object should be of type: Vector, Line, Circle, Rectangle or Polygon');}};Vector.prototype.drawCanvas=function(c,cameraOffset){c.save();c.translate(-cameraOffset.x,-cameraOffset.y);c.fillStyle='#f00';c.beginPath();c.moveTo(this.x,this.y);c.arc(this.x,this.y,2,0,Math.PI*2,true);c.fill();c.restore();};NewClass('View',[Vector]);View.prototype.View=function(){this.Vector();this.children=[];this.drawCacheCanvas=document.createElement('canvas');this.drawCacheCtx=this.drawCacheCanvas.getContext('2d');this.drawCacheEnabled=false;this.drawCacheOffset=new Vector();};View.prototype.getRoomPosition=function(){var pos;pos=this.copy();parent=this;while((parent=parent.parent)!==undefined){pos.move(parent.x,parent.y);}
return pos;};View.prototype.addChildren=function(child1,child2){if(arguments.length===0){throw new Error('This function needs at least one argument');}
...
var i;func.call(this);for(i=0;i<this.children.length;i++){this.children[i].applyToThisAndChildren(func);}};View.prototype.setDrawCache=function(enabled){enabled=enabled===true;if(enabled===this.drawCacheEnabled){return;}
if(enabled){this.cacheDrawing();}
this.drawCacheEnabled=enabled;};View.prototype.cacheDrawing=function(){var drawRegion;drawRegion=this.getCombinedRedrawRegion();drawRegion.move(-this.x,-this.y);if(drawRegion){this.drawCacheOffset=new Vector(drawRegion.x,drawRegion.y);this.drawCacheCanvas.width=drawRegion.width;this.drawCacheCanvas.height=drawRegion.height;this.draw(this.drawCacheCtx,this.drawCacheOffset.copy().move(this.x,this.y),true);}};View.prototype.getCombinedRedrawRegion=function(){var box,addBox,i;if(this.getRedrawRegion){box=this.getRedrawRegion();}
for(i=0;i<this.children.length;i++){addBox=this.children[i].getCombinedRedrawRegion();if(addBox){if(box){box=box.combine(addBox);}
else{box=addBox;}}}
return box;};View.prototype.removeChildren=function(child1,child2){if(arguments.length===0){throw new Error('This function needs at least one argument');}
...
i--;}
return removed;};View.prototype.removeAllChildren=function(purge){purge=purge!==undefined?purge:true;var rmChild;rmChild=this.children.splice(0,this.children.length);rmChild.forEach(function(){this.parent=undefined;if(purge){this.remove(purge);}});};View.prototype.draw=function(c,cameraOffset,forceRedraw){var i,len,child,newOffset;if(this.drawCacheEnabled&&!forceRedraw){c.drawImage(this.drawCacheCanvas,this.x+this.drawCacheOffset.x-cameraOffset.x,this.y+this.drawCacheOffset.y-cameraOffset.y,this.drawCacheCanvas.width,this.drawCacheCanvas.height);}
...
else if(child.drawCanvas){child.draw(c,newOffset);}}}};View.prototype.drawCanvas=undefined;NewClass('Room',[View]);Room.prototype.Room=function(name,onEntered,onLeft){this.View();this.name=name?name:engine.roomList.length;this.onEntere=onEntered!==undefined?onEntered:function(){};this.onLeft=onLeft!==undefined?onLeft:function(){};this.loops={};this.addLoop('eachFrame',new CustomLoop());engine.addRoom(this);};Room.prototype.update=function(){var i;for(i in this.loops){if(this.loops.hasOwnProperty(i)){this.loops[i].execute();}}};Room.prototype.addLoop=function(name,loop){if(loop===undefined){throw new Error('Missing argument: loop');}
if(name===undefined){throw new Error('Missing argument: name');}
if(this.loops[name]!==undefined){throw new Error('Name is taken: '+name);}
this.loops[name]=loop;};Room.prototype.removeLoop=function(name){if(name===undefined){throw new Error('Missing argument: name');}
if(name==='eachFrame'){throw new Error('The "eachFrame" loop cannot be removed');}
delete this.loops[name];};Room.prototype.remove=undefined;NewClass('Line');Line.prototype.Line=function(startVector,endVector){startVector=startVector!==undefined?startVector:new Vector(0,0);endVector=endVector!==undefined?endVector:new Vector(0,0);this.setFromVectors(startVector,endVector);};Line.prototype.setFromVectors=function(startVector,endVector){if(!startVector.implements(Vector)){throw new Error('Agument startVector should be of type: Vector');}
...
else{throw new Error('Agument object should be of type: Vector, Line, Circle, Rectangle or Polygon');}};Line.prototype.drawCanvas=function(c,cameraOffset){c.save();c.translate(-cameraOffset.x,-cameraOffset.y);c.strokeStyle="#f00";c.beginPath();c.moveTo(this.a.x,this.a.y);c.lineTo(this.b.x,this.b.y);c.stroke();c.restore();};NewClass('Rectangle',[Animatable,Vector]);Rectangle.prototype.Rectangle=function(x,y,width,height){this.set(x,y,width,height);};Rectangle.prototype.set=function(x,y,width,height){this.x=x!==undefined?x:0;this.y=y!==undefined?y:0;this.width=width!==undefined?width:0;this.height=height!==undefined?height:0;return this;};Rectangle.prototype.setFromVectors=function(position,size){position=position!==undefined?position:new Vector();size=size!==undefined?size:new Vector();this.x=position.x;this.y=position.y;this.width=size.x;this.height=size.y;return this;};Rectangle.prototype.copy=function(){return new Rectangle(this.x,this.y,this.width,this.height);};Rectangle.prototype.move=function(x,y){if(typeof x!=='number'){throw new Error('Argument x should be of type: Number');}
if(typeof y!=='number'){throw new Error('Argument y should be of type: Number');}
...
if(typeof y!=='number'){throw new Error('Argument y should be of type: Number');}
this.x=x;this.y=y;return this;};Rectangle.prototype.scale=function(scaleH,scaleV){if(typeof scaleH!=='number'){throw new Error('Argument scaleH should be of type Number');}
scaleV=scaleV!==undefined?scaleV:scaleH;this.width*=scaleH;this.height*=scaleV;return this;};Rectangle.prototype.combine=function(rectangle){if(!rectangle.implements(Rectangle)){throw new Error('Argument rectangle should be of type Rectangle');}
var pol=this.getPolygon();pol.points=pol.points.concat(rectangle.getPolygon().points);return pol.getBoundingRectangle();};Rectangle.prototype.getPolygon=function(){return new Polygon(this.getPoints());};Rectangle.prototype.getPoints=function(){return[new Vector(this.x,this.y),new Vector(this.x+this.width,this.y),new Vector(this.x+this.width,this.y+this.height),new Vector(this.x,this.y+this.height)];};Rectangle.prototype.getArea=function(){return this.width*this.height;};Rectangle.prototype.getDiagonal=function(){return Math.sqrt(Math.pow(this.width,2)+Math.pow(this.height,2));};Rectangle.prototype.getDistance=function(object){return this.getPolygon().getDistance(object);};Rectangle.prototype.contains=function(object){return this.getPolygon().contains(object);};Rectangle.prototype.intersects=function(object){return this.getPolygon().intersects(object);};Rectangle.prototype.drawCanvas=function(c,cameraOffset){c.save();c.translate(-cameraOffset.x,-cameraOffset.y);c.strokeStyle="#f00";c.beginPath();c.moveTo(this.x,this.y);c.lineTo(this.x+this.width,this.y);c.lineTo(this.x+this.width,this.y+this.height);c.lineTo(this.x,this.y+this.height);c.lineTo(this.x,this.y);c.stroke();c.closePath();c.restore();};NewClass('Polygon');Polygon.prototype.Polygon=function(points){this.setFromPoints(points);};Polygon.prototype.setFromPoints=function(points){this.points=points;return this;};Polygon.prototype.setFromCoordinates=function(x1,y1,x2,y2,x3,y3){var numPoints,i,x,y;numPoints=Math.floor(arguments.length/2);this.points=[];for(i=0;i<numPoints;i++){x=arguments[i*2];y=arguments[i*2+1];if(typeof x!=='number'||typeof y!=='number'){throw new Error('All arguments should be of type: Number');}
this.points.push(new Vector(x,y));}
return this;};Polygon.prototype.move=function(x,y){if(typeof x!=='number'){throw new Error('Argument x should be of type Number');}
if(typeof y!=='number'){throw new Error('Argument y should be of type Number');}
return this.add(new Vector(x,y));};Polygon.prototype.add=function(vector){if(!vector.implements(Vector)){throw new Error('Argument vector should be of type Vector');}
var i;for(i=0;i<this.points.length;i++){this.points[i].add(vector);}
return this;};Polygon.prototype.rotate=function(dir){if(typeof dir!=='number'){throw new Error('Argument dir should be of type Number');}
var i;for(i=0;i<this.points.length;i++){this.points[i].rotate(dir);}
return this;};Polygon.prototype.scale=function(scaleH,scaleV){var i;for(i=0;i<this.points.length;i++){this.points[i].scale(scaleH,scaleV);}
...
return lines;};Polygon.prototype.getBoundingRectangle=function(){if(this.points.length===0){throw new Error('Cannot create bounding rectangle for pointless polygon');}
...
return new Rectangle().setFromVectors(startVector,endVector.subtract(startVector));};Polygon.prototype.getDistance=function(object){var dist,lines,objLines,i,ii,pVector;dist=2E+10308;lines=this.getLines();if(object.implements(Vector)){for(i=0;i<lines.length;i++){dist=Math.min(dist,lines[i].getDistance(object));if(dist<0){break;}}
return dist;}
...
if(dist<0){break;}}
return dist;}
else{throw new Error('Agument object should be of type: Vector, Line, Circle, Rectangle or Polygon');}};Polygon.prototype.contains=function(object){if(object.implements(Vector)){if(this.intersects(new Line().setFromCoordinates(-123456,-98765,object.x,object.y),true)%2){return true;}
else{return false;}}
...
else{throw new Error('Argument object has to be of type: Vector, Line, Rectangle or Polygon');}};Polygon.prototype.intersects=function(object,countIntersections){var intersects,intersectionCount,lines,line,oLines,oLine,i,ii;intersects=false;countIntersections=countIntersections!==undefined?countIntersections:false;if(countIntersections){intersectionCount=0;}
...
else{return false;}};Polygon.prototype.drawCanvas=function(c,cameraOffset){var i,len;c.save();c.translate(-cameraOffset.x,-cameraOffset.y);c.strokeStyle="#f00";c.beginPath();len=this.points.length;c.moveTo(this.points[len-1].x,this.points[len-1].y);for(i=0;i<len;i++){c.lineTo(this.points[i].x,this.points[i].y);}
c.stroke();c.restore();};NewClass('Circle',[Animatable]);Circle.prototype.Circle=function(x,y,radius){this.set(x,y,radius);};Circle.prototype.set=function(x,y,radius){x=x!==undefined?x:0;y=y!==undefined?y:0;radius=radius!==undefined?radius:0;this.x=x;this.y=y;this.radius=radius;return this;};Circle.prototype.copy=function(){return new Circle(this.x,this.y,this.radius);};Circle.prototype.move=function(x,y){if(typeof x!=='number'){throw new Error('Argument x should be of type: Number');}
if(typeof y!=='number'){throw new Error('Argument y should be of type: Number');}
...
this.radius*=factor;return this;};Circle.prototype.getArea=function(){return Math.pow(this.radius)*Math.PI;};Circle.prototype.getDistance=function(object){if(object.implements(Vector)){return Math.max(0,object.getDistance(new Vector(this.x,this.y))-this.radius);}
...
else{throw new Error('Agument object should be of type: Vector, Line, Circle, Rectangle or Polygon');}};Circle.prototype.contains=function(object){var i,cDist;if(object.implements(Vector)){return object.copy().move(-this.x,-this.y).getLength()<this.radius;}
...
else{throw new Error('Argument object has to be of type: Line, Circle, Rectangle or Polygon');}};Circle.prototype.drawCanvas=function(c,cameraOffset){c.save();c.translate(-cameraOffset.x,-cameraOffset.y);c.strokeStyle="#f00";c.beginPath();c.arc(this.x,this.y,this.radius,0,Math.PI*2,true);c.stroke();c.restore();};NewClass('Camera');Camera.prototype.Camera=function(captureRegion,projectionRegion){if(!captureRegion.implements(Rectangle)){throw new Error('Argument captureRegion should be of type: Rectangle');}
if(!projectionRegion.implements(Rectangle)){throw new Error('Argument projectionRegion should be of type: Rectangle');}
this.captureRegion=captureRegion;this.projectionRegion=projectionRegion;this.canvas=document.createElement('canvas');this.canvas.width=this.captureRegion.width;this.canvas.height=this.captureRegion.height;this.room=engine.currentRoom;this.ctx=this.canvas.getContext('2d');};Camera.prototype.updateCaptureCanvas=function(){if(this.captureRegion.width!==this.canvas.width){this.canvas.width=this.captureRegion.width;}
if(this.captureRegion.height!==this.canvas.height){this.canvas.height=this.captureRegion.height;}};Camera.prototype.capture=function(){this.updateCaptureCanvas();this.ctx.fillStyle=engine.backgroundColor;this.ctx.fillRect(0,0,this.captureRegion.width,this.captureRegion.height);engine.masterRoom.draw(this.ctx,this.captureRegion.copy());this.room.draw(this.ctx,this.captureRegion.copy());};Camera.prototype.draw=function(c){c.save();c.drawImage(this.canvas,this.projectionRegion.x,this.projectionRegion.y,this.projectionRegion.width,this.projectionRegion.height);c.restore();};NewClass('Loader');Loader.prototype.Loader=function(){this.sounds={};this.images={};this.scripts={};this.loaded={classes:[]};this.themes={};this.callback=false;this.loadOverlay=document.createElement('div');this.loadOverlay.setAttribute('style','border: 0;position: absolute;top: 0;left: 0;width: 100%;height: 100%;z-index: 100;opacity: 1;');this.loadOverlay.id="loadOverlay";this.loadOverlay.innerHTML='<div id="loadOverlayText">'+engine.loadText+'</div>';engine.arena.appendChild(this.loadOverlay);};Loader.prototype.hideOverlay=function(callback){this.fadeCallback=callback;this.fadeOpacity=1;this.fade=function(){var obj=loader.loadOverlay;loader.fadeOpacity=Math.max(0,loader.fadeOpacity-0.1);loader.fadeOpacity=Math.floor(loader.fadeOpacity*100)/100;obj.style.opacity=loader.fadeOpacity;if(loader.fadeOpacity!==0){setTimeout(function(){loader.fade();},30);}
else{engine.arena.removeChild(loader.loadOverlay);delete loader.fade;if(loader.fadeCallback){loader.fadeCallback();}
delete loader.fadeCallback;}};this.fade();};Loader.prototype.getImage=function(resource,themeName){if(resource===undefined){throw new Error('Missing argument: resource');}
themeName=themeName!==undefined?themeName:engine.defaultTheme;return this.getResource(resource,'images',themeName);};Loader.prototype.getSound=function(resource,themeName){if(resource===undefined){throw new Error('Missing argument: resource');}
themeName=themeName!==undefined?themeName:engine.defaultTheme;return this.getResource(resource,'sfx',themeName);};Loader.prototype.getMusic=function(resource,themeName){if(resource===undefined){throw new Error('Missing argument: resource');}
themeName=themeName!==undefined?themeName:engine.defaultTheme;return this.getResource(resource,'music',themeName);};Loader.prototype.getMask=function(resource,themeName){if(resource===undefined){throw new Error('Missing argument: resource');}
...
else{mask=this.generateMask(resource);this.themes[themeName].masks[resource]=mask;return mask;}};Loader.prototype.getResource=function(resource,typeString,themeName){if(resource===undefined){throw new Error('Missing argument: resource');}
...
return sourceStrings;};Loader.prototype.getAllSounds=function(){var res,themeName,theme,resourceString,resource;res=[];for(themeName in this.themes){if(this.themes.hasOwnProperty(themeName)){theme=this.themes[themeName];for(resourceString in theme.sfx){if(theme.sfx.hasOwnProperty(resourceString)){res.push(theme.sfx[resourceString]);}}}}
return res;};Loader.prototype.getAllMusic=function(){var res,themeName,theme,resourceString,resource;res=[];for(themeName in this.themes){if(this.themes.hasOwnProperty(themeName)){theme=this.themes[themeName];for(resourceString in theme.music){if(theme.music.hasOwnProperty(resourceString)){res.push(theme.music[resourceString]);}}}}
return res;};Loader.prototype.loadClasses=function(paths){if(paths===undefined){throw new Error('Missing argument: paths');}
...
return true;};Loader.prototype.reloadAllClasses=function(){var i;for(i in this.loaded.classes){if(this.loaded.classes.hasOwnProperty(i)){engine.loadFiles(this.loaded.classes[i]);}}};Loader.prototype.loadThemes=function(themeNames,callback){if(themeNames===undefined){throw new Error('Missing argument: themeNames');}
...
if(total===0){if(this.onthemesloaded){this.onthemesloaded();}}};Loader.prototype.loadResources=function(theme,object,typeString){if(theme===undefined){throw new Error('Missing argument: theme');}
...
if(engine.preGenerateBoundingBoxes&&this.toString()==='[object HTMLImageElement]'){resourceString=this.getAttribute('data-resourceString');theme=this.getAttribute('data-theme');console.log('Pre-generating bounding box: '+resourceString);for(i=0;i<100;i++){loader.getBBox(resourceString,theme,i/50*Math.PI);}}
...
break;}
res.setAttribute('data-theme',theme.name);res.setAttribute('data-resourceString',path.replace(/\./g,'/'));}}};Loader.prototype.generateMask=function(resourceString,alphaLimit){if(resourceString===undefined){throw new Error('Missing argument: resourceString');}
alphaLimit=alphaLimit!==undefined?alphaLimit:255;var image,canvas,ctx,bitmap,data,length,pixel,top,bottom,left,right,x,y;image=loader.getImage(resourceString);canvas=document.createElement('canvas');canvas.width=image.width;canvas.height=image.height;ctx=canvas.getContext('2d');if(image===false){throw new Error('Trying to create mask for non-existing resource: '+resourceString);}
...
return true;}
return false;};NewClass('CustomLoop');CustomLoop.prototype.CustomLoop=function(framesPerExecution,maskFunction){this.framesPerExecution=framesPerExecution===undefined?1:framesPerExecution;this.maskFunction=maskFunction===undefined?function(){return true;}:maskFunction;this.functionsQueue=[];this.functions=[];this.executionsQueue=[];this.executions=[];this.animations=[];this.lastFrame=engine.frames;this.last=engine.now?engine.now:new Date().getTime();this.time=0;this.execTime=0;};CustomLoop.prototype.attachFunction=function(caller,func){if(caller===undefined){throw new Error('Missing argument: caller');}
...
this.functionsQueue.push({object:caller,activity:func});};CustomLoop.prototype.addFunctionsQueue=function(){this.functions=this.functions.concat(this.functionsQueue);this.functionsQueue=[];};CustomLoop.prototype.detachFunction=function(caller,func){if(caller===undefined){throw new Error('Missing argument: caller');}
if(func===undefined){throw new Error('Missing argument: func');}
...
return false;};CustomLoop.prototype.detachFunctionsByFunction=function(func){if(func===undefined){throw new Error('Missing argument: func');}
...
else{return false;}};CustomLoop.prototype.detachFunctionsByCaller=function(caller){if(caller===undefined){throw new Error('Missing argument: caller');}
...
if(removeArray.length){return removeArray;}
else{return false;}};CustomLoop.prototype.schedule=function(caller,func,delay){if(caller===undefined){throw new Error('Missing argument: caller');}
if(func===undefined){throw new Error('Missing argument: function');}
if(delay===undefined){throw new Error('Missing argument: delay');}
this.executionsQueue.push({func:func,execTime:this.time+delay,caller:caller,});};CustomLoop.prototype.addExecutionsQueue=function(){this.executions=this.executions.concat(this.executionsQueue);this.executionsQueue=[];};CustomLoop.prototype.unSchedule=function(caller,func){if(caller===undefined){throw new Error('Missing argument: caller');}
if(func===undefined){throw new Error('Missing argument: function');}
...
return false;};CustomLoop.prototype.unScheduleByFunction=function(func){if(func===undefined){throw new Error('Missing argument: func');}
...
else{return false;}};CustomLoop.prototype.unScheduleByCaller=function(caller){if(caller===undefined){throw new Error('Missing argument: caller');}
...
if(unscheduledArray.length){return unscheduledArray;}
else{return false;}};CustomLoop.prototype.unScheduleAll=function(){var removeArray;removeArray=[].concat(this.executions,this.executionsQueue);this.executions=[];this.executionsQueue=[];return removeArray;};CustomLoop.prototype.addAnimation=function(animation){if(animation===undefined){throw new Error('Missing argument: animation');}
...
this.animations.push(anim);};CustomLoop.prototype.removeAnimationsOfObject=function(object){i=this.animations.length;while(i--){if(object===this.animations[i].obj){this.animations.splice(i,1);}}};CustomLoop.prototype.updateAnimations=function(){var animId,a,propId,t;for(animId=this.animations.length-1;animId>-1;animId--){a=this.animations[animId];if(a===undefined){continue;}
...
if(typeof a.callback==="string"){eval(a.callback);}else{a.callback.call(a.obj);}}else{for(propId in a.prop){if(a.prop.hasOwnProperty(propId)){a.obj[propId]=engine.ease(a.easing,t,a.prop[propId].begin,a.prop[propId].end-a.prop[propId].begin,a.dur);}}}}};CustomLoop.prototype.execute=function(){var timer,i,exec;timer=new Date().getTime();if(!this.maskFunction()||engine.frames%this.framesPerExecution){return;}
...
this.addFunctionsQueue();this.addExecutionsQueue();this.execTime=(new Date().getTime())-timer;};NewClass('Sprite',[View,Animatable]);Sprite.prototype.Sprite=function(source,x,y,dir,additionalProperties){if(source===undefined){throw new Error('Missing argument: source');}
...
return theme?theme:engine.defaultTheme;};Sprite.prototype.refreshSource=function(){var theme;theme=this.getTheme();this.bm=loader.getImage(this.source,theme);this.imageLength=this.bm.imageLength*1;this.imageNumber=Math.min(this.imageLength-1,this.imageNumber);this.width=Math.floor(this.bm.width/this.imageLength);this.height=this.bm.height;return this.bm;};Sprite.prototype.setSource=function(source){if(source===undefined){throw new Error('Missing argument: source');}
if(this.source===source){return;}
this.source=source;this.refreshSource();};Sprite.prototype.setWidth=function(width){this.widthModifier=width/(this.width*this.size);};Sprite.prototype.setHeight=function(height){this.heightModifier=height/(this.height*this.size);};Sprite.prototype.drawCanvas=function(c,cameraOffset){var x,y;if(engine.avoidSubPixelRendering){x=Math.round(this.x-cameraOffset.x);y=Math.round(this.y-cameraOffset.y);}
...
if(this.dir!==0){c.translate(x,y);c.rotate(this.dir);c.drawImage(this.bm,(this.width+this.bm.spacing)*this.imageNumber,0,this.width,this.height,-this.offset.x*this.size*this.widthModifier,-this.offset.y*this.size*this.heightModifier,this.width*this.size*this.widthModifier,this.height*this.size*this.heightModifier);c.rotate(-this.dir);c.translate(-x,-y);}
else{c.drawImage(this.bm,(this.width+this.bm.spacing)*this.imageNumber,0,this.width,this.height,x-this.offset.x*this.size*this.widthModifier,y-this.offset.y*this.size*this.heightModifier,this.width*this.size*this.widthModifier,this.height*this.size*this.heightModifier);}
if(this.composite!=='source-over'){c.globalCompositeOperation='source-over';}
c.globalAlpha=1;};Sprite.prototype.getRedrawRegion=function(){var ret;ret=new Rectangle(-this.offset.x,-this.offset.y,Math.floor(this.bm.width/this.imageLength),this.bm.height);ret=ret.getPolygon();ret=ret.scale(this.size*this.widthModifier,this.size*this.heightModifier);ret=ret.rotate(this.dir);ret=ret.getBoundingRectangle().add(this.getRoomPosition());ret.x=Math.floor(ret.x-1);ret.y=Math.floor(ret.y-1);ret.width=Math.ceil(ret.width+2);ret.height=Math.ceil(ret.height+2);return ret;};NewClass('Collidable',[Sprite]);Collidable.prototype.Collidable=function(source,x,y,dir,additionalProperties){this.Sprite(source,x,y,dir,additionalProperties);this.mask=this.mask?this.mask:loader.getMask(source,this.getTheme());this.collisionResolution=this.collisionResolution?this.collisionResolution:engine.defaultCollisionResolution;};Collidable.prototype.boundingBoxCollidesWith=function(objects,getCollidingObjects){if(objects===undefined){throw new Error('Missing argument: objects');}
...
else{return false;}};Collidable.prototype.maskCollidesWith=function(objects,getCollisionPosition){if(objects===undefined){throw new Error('Missing argument: objects');}
var canvas,mask,ctx,roomPos,obj,bitmap,i,data,length,pixel,pxArr,x,y,avX,avY,avDist,avDir;if(!Array.prototype.isPrototypeOf(objects)){objects=[objects];}
getCollisionPosition=getCollisionPosition!==undefined?getCollisionPosition:false;mask=loader.getMask(this.source,this.getTheme());canvas=document.createElement('canvas');canvas.width=Math.ceil(mask.width*this.size*this.widthModifier);canvas.height=Math.ceil(mask.height*this.size*this.heightModifier);ctx=canvas.getContext('2d');ctx.fillStyle="#FFF";ctx.fillRect(0,0,canvas.width,canvas.height);ctx.globalAlpha=0.5;ctx.save();ctx.drawImage(mask,(this.width+this.bm.spacing)*this.imageNumber,0,this.width,this.height,0,0,this.width*this.size*this.widthModifier,this.height*this.size*this.heightModifier);ctx.translate(this.offset.x*this.size*this.widthModifier,this.offset.y*this.size*this.heightModifier);ctx.rotate(-this.dir);roomPos=this.getRoomPosition();for(i=0;i<objects.length;i++){obj=objects[i];if(obj===this){continue;}
mask=loader.getMask(obj.source,obj.getTheme());ctx.translate(obj.x-roomPos.x,obj.y-roomPos.y);ctx.rotate(obj.dir);ctx.drawImage(obj.mask,(obj.width+obj.bm.spacing)*obj.imageNumber,0,obj.width,obj.height,-obj.offset.x*obj.size*obj.widthModifier,-obj.offset.y*obj.size*obj.heightModifier,obj.width*obj.size*obj.widthModifier,obj.height*obj.size*obj.heightModifier);ctx.rotate(-obj.dir);ctx.translate(roomPos.x-obj.x,roomPos.y-obj.y);}
bitmap=ctx.getImageData(0,0,canvas.width,canvas.height);data=bitmap.data;length=data.length/4;pxArr=[];for(pixel=0;pixel<length;pixel+=this.collisionResolution){if(data[pixel*4]<127){if(getCollisionPosition){y=Math.floor(pixel/bitmap.width);x=pixel-y*bitmap.width;pxArr.push({x:x,y:y});}
else{return true;}}}
if(pxArr.length>0){pxArr=pxArr.sortByNumericProperty('x');avX=(pxArr[0].x+pxArr[pxArr.length-1].x)/2;pxArr=pxArr.sortByNumericProperty('y');avY=(pxArr[0].y+pxArr[pxArr.length-1].y)/2;avX-=this.offset.x*this.size*this.widthModifier;avY-=this.offset.y*this.size*this.heightModifier;avDir=Math.atan2(avY,avX);avDist=Math.sqrt(Math.pow(avX,2)+Math.pow(avY,2));avDir+=this.dir;avX=Math.cos(avDir)*avDist;avY=Math.sin(avDir)*avDist;return{x:avX,y:avY,direction:avDir};}
return false;};Collidable.prototype.collidesWith=function(objects,getCollisionPosition,getCollidingObjects){if(objects===undefined){throw new Error('Missing argument: objects');}
...
else{ret={objects:[],positions:[]};if(getCollidingObjects===false){position=this.maskCollidesWith(objects,true);if(position){ret.positions.push(position);}}
...
else{return false;}};Collidable.prototype.drawBoundingBox=function(c,cameraOffset){var pol;pol=this.mask.bBox.copy().move(this.mask.width/2-this.offset.x,this.mask.height/2-this.offset.y).scale(this.size*this.widthModifier,this.size*this.heightModifier).rotate(this.dir);c.save();c.strokeStyle="#0F0";c.translate(this.x-cameraOffset.x,this.y-cameraOffset.y);c.beginPath();c.moveTo(pol.points[0].x,pol.points[0].y);c.lineTo(pol.points[1].x,pol.points[1].y);c.lineTo(pol.points[2].x,pol.points[2].y);c.lineTo(pol.points[3].x,pol.points[3].y);c.closePath();c.stroke();c.restore();};Collidable.prototype.drawMask=function(c,cameraOffset){var mask;if(this.mask){mask=this.mask;}
else{return;}
c.save();c.translate(this.x-cameraOffset.x,this.y-cameraOffset.y);c.rotate(this.dir);try{c.drawImage(this.mask,(this.width+this.bm.spacing)*this.imageNumber,0,this.width,this.height,-this.offset.x*this.size*this.widthModifier,-this.offset.y*this.size*this.heightModifier,this.width*this.size*this.widthModifier,this.height*this.size*this.heightModifier);}catch(e){console.log(this.source);console.log(this.bm);engine.stopMainLoop();throw new Error(e);}
c.restore();};NewClass('TextBlock',[Animatable,View]);TextBlock.prototype.TextBlock=function(string,x,y,width,additionalProperties){if(string===undefined){throw new Error('Missing argument: string');}
...
this.color=colorString;this.cacheRendering();};TextBlock.prototype.cacheRendering=function(){var xOffset,i;this.bmCtx.clearRect(0,0,this.bm.width,this.bm.height);this.bmCtx.font=this.font;this.bmCtx.fillStyle=this.color;for(i=0;i<this.lines.length;i++){xOffset=0;switch(this.alignment){case'left':xOffset=0;break;case'right':xOffset=this.width-this.lineWidth[i];break;case'center':xOffset=(this.width-this.lineWidth[i])/2;break;}
if(this.lines[i]){this.bmCtx.fillText(this.lines[i],xOffset,this.lineHeight*(1+i));}}};TextBlock.prototype.stringToLines=function(){var lt,line,paragraphs,pid,words,wid,word;lt=document.createElement('span');lt.style.font=this.font;lt.style.visibility='hidden';lt.style.position='absolute';document.body.appendChild(lt);line=0;this.lines=[];this.lines[line]='';paragraphs=this.string.split("\n");for(pid=0;pid<paragraphs.length;pid++){words=paragraphs[pid].split(' ');for(wid=0;wid<words.length;wid++){word=words[wid];lt.innerHTML+=word+" ";if(lt.offsetWidth>this.width){line++;this.lines[line]='';lt.innerHTML='';lt.innerHTML+=word+" ";}
else{this.lineWidth[line]=lt.offsetWidth;}
this.lines[line]+=word+" ";}
line++;lt.innerHTML='';this.lines[line]='';}
lt.parentNode.removeChild(lt);this.bm.height=this.lines.length*this.lineHeight;};TextBlock.prototype.setWidth=function(width){this.widthModifier=width/(this.bm.width*this.size);};TextBlock.prototype.setHeight=function(height){this.heightModifier=height/(this.bm.height*this.size);};TextBlock.prototype.drawCanvas=function(c,cameraOffset){var x,y;if(/^\s*$/.test(this.string)){return;}
if(engine.avoidSubPixelRendering){x=Math.round(this.x-cameraOffset.x);y=Math.round(this.y-cameraOffset.y);}
else{x=this.x-cameraOffset.x;y=this.y-cameraOffset.y;}
c.globalAlpha=this.opacity;if(this.composite!=='source-over'){c.globalCompositeOperation=this.composite;}
if(this.dir!==0){c.save();c.translate(x,y);c.rotate(this.dir);c.drawImage(this.bm,-this.offset.x*this.size*this.widthModifier,-this.offset.y*this.size*this.heightModifier,this.bm.width*this.size*this.widthModifier,this.bm.height*this.size*this.heightModifier);c.rotate(-this.dir);c.translate(-x,-y);}
else{c.drawImage(this.bm,x-this.offset.x*this.size*this.widthModifier,y-this.offset.y*this.size*this.heightModifier,this.bm.width*this.size*this.widthModifier,this.bm.height*this.size*this.heightModifier);}
if(this.composite!=='source-over'){c.globalCompositeOperation='source-over';}
c.globalAlpha=1;};TextBlock.prototype.getRedrawRegion=function(){var ret;ret=new Rectangle(-this.offset.x,-this.offset.y,this.bm.width,this.bm.height);ret=ret.getPolygon();ret=ret.scale(this.size*this.widthModifier,this.size*this.heightModifier);ret=ret.rotate(this.dir);ret=ret.getBoundingRectangle().add(this.getRoomPosition());ret.x=Math.floor(ret.x-1);ret.y=Math.floor(ret.y-1);ret.width=Math.ceil(ret.width+2);ret.height=Math.ceil(ret.height+2);return ret;};NewClass('GameObject',[Collidable]);GameObject.prototype.GameObject=function(source,x,y,dir,additionalProperties){if(source===undefined){throw new Error('Missing argument: source');}
if(x===undefined){throw new Error('Missing argument: x');}
if(y===undefined){throw new Error('Missing argument: y');}
this.Collidable(source,x,y,dir,additionalProperties);this.loop=this.loop?this.loop:engine.currentRoom.loops.eachFrame;this.loop.attachFunction(this,this.updatePosition);this.speed=this.speed?this.speed:new Vector(0,0);this.alive=true;};GameObject.prototype.updatePosition=function(){if(this.alive){this.add(engine.convertSpeed(this.speed));}};NewClass('Keyboard');Keyboard.prototype.Keyboard=function(){var key;document.addEventListener('keydown',function(event){keyboard.onKeyDown(event);event.preventDefault();return false;},false);document.addEventListener('keyup',function(event){keyboard.onKeyUp(event);event.preventDefault();return false;},false);this.keys=new Array(200);for(key=0;key<this.keys.length;key++){this.keys[key]={events:[]};}};Keyboard.prototype.onKeyDown=function(event){var key;if(event===undefined){throw new Error('Missing argument: event');}
...
if(typeof key==='string'){key=key.toUpperCase().charCodeAt(0);}
return this.keys[key].events.length&&-this.keys[key].events[0]>engine.last;};NewClass('Pointer');Pointer.prototype.Pointer=function(){var button;if(engine.host.hasTouch){engine.arena.addEventListener('touchstart',function(event){pointer.onTouchStart(event);},false);engine.arena.addEventListener('touchend',function(event){pointer.onTouchEnd(event);},false);document.addEventListener('touchmove',function(event){pointer.onTouchMove(event);},false);}
...
break;}}
return ret.length?ret:false;};Pointer.prototype.calculateRoomPosition=function(vector){var ret,len,camera;ret=vector.copy();len=engine.cameras.length;while(len--){camera=engine.cameras[len];if(camera.projectionRegion.contains(vector)){ret.subtract(camera.projectionRegion);ret.x*=camera.captureRegion.width/camera.projectionRegion.width;ret.y*=camera.captureRegion.height/camera.projectionRegion.height;ret.add(camera.captureRegion);return ret;}}
ret.set(false,false);return ret;};Pointer.prototype.findTouchNumber=function(){var i;for(i=0;i<this.touches.length;i++){if(!this.checkPointer(this.touches[i],'down')){return i;}}};Pointer.prototype.shapeIsHovered=function(shape,outside){if(!outside&&(shape.contains(this.mouse))){return true;}
...
return true;};Pointer.prototype.outside=function(){return this.mouse.x===false||this.mouse.y===false;};NewClass('Sound');Sound.prototype.Sound=function(audioElement){if(audioElement===undefined){throw new Error('Missing argument: audioElement');}
...
var i,snd;this.source=audioElement;this.playbackId=0;this.elements=[];snd=this;this.source.addEventListener("canplaythrough",function(){snd.cacheCopies();},false);};Sound.prototype.cacheCopies=function(){for(i=0;i<engine.cachedSoundCopies;i++){this.elements.push(this.source.cloneNode());this.elements[i].started=false;}};Sound.prototype.play=function(loop){var i,sound;if(engine.soundsMuted){return false;}
...
return false;};NewClass('Music');Music.prototype.Music=function(audioElement){if(audioElement===undefined){throw new Error('Missing argument: audioElement');}
if(audioElement.toString()!=="[object HTMLAudioElement]"){throw new Error('Argument audioElement has to be of type HTMLAudioElement');}
